...
@Service
@Slf4j
public class FileDataService extends BaseService<FileData, Integer> {
    //用来记录日志
    private Logger logger = LoggerFactory.getLogger(this.getClass());

    //得到查询结果
    @Transactional
    public Page getSearch(FileDataDTO para, int pageIndex, int pageSize) throws Exception {
        //设置查询条件
        List<SearchField> condition = new ArrayList<SearchField>();

        //设置关联表的查询 lift inner join orgs left join users
        this.setTableName("FileData");

        //设置查询条件
        if (MyStringUtil.isNotEmpty(para.getFileTitle())) {
            condition.add(new SearchField("fileTitle", "%" + para.getFileTitle() + "%", SearchOperator.Contains));
        }
        ...
        if (MyStringUtil.isNotEmpty(para.getFileKind())) {
            condition.add(new SearchField("fileKind", para.getFileKind()));
        }

        //返回查询的列
        String fs = "id,fileKind,fileTitle,fileName,filedir,filekeys,remark";
