...
//            reader = null;
    ...
    public static String postFileLocal(MultipartFile mfile, String fileFolder) {
        if (mfile != null) {
            String key = UUID.randomUUID().toString();
            String fileName = fileFolder + "/" + key.substring(0, 8) + "_" + mfile.getOriginalFilename();
            File file = new File(SystemConfig.getInstance().getFileDir() + fileName);
    ...
    public static void saveFileFromInputStream(InputStream stream, File file)
            throws IOException {
